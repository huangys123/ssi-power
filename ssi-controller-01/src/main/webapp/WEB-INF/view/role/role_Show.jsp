	...
	<script type="text/javascript">
		...
		function init_role_dg() {
			...
			$('#role_dg').bootstrapTable({
				url:"<%=request.getContextPath() %>/selectRoleListJson2.jhtml",
				dataType:"json",
				//请求方式
				method:"post",
				//必须的，操你大爷！！！！不然会造成中文乱码
			    ...
			    columns: [{
			        ...
			        formatter:function(value, row, index) {
			        	...
			        	+ '</div>&nbsp;&nbsp;'
			        	+ '<div class="btn-group">'
			        	+ '<button type="button" class="btn btn-xs btn-success" onclick="edit_role_menu(\'' + row.roleID + '\')">角色操作</button>'
			        	+ '</div>';
			        	return zc_btn_group;
			        }
			    },]
			...
			BootstrapDialog.show({
				...
				buttons: [{
	                ...
	                action: function(dialogItself){
	                	...
	                	//使用ajax保存结果
	                	$.ajax({
	                		url:"<%=request.getContextPath() %>/insertRoleMenuList.jhtml",
